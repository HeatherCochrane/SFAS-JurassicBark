...
public class Environment : MonoBehaviour
{
    ...
    private void Generate()
    {
        ...
        for ( int x = 0; x < Size.x; ++x)
        {
            ...
            for ( int y = 0; y < Size.y; ++y)
            {
				...
				//Set the first tile to be the starting point
                if(start)
                {
                    Start = tile;
                }
    ...
    public List<EnvironmentTile> Solve(EnvironmentTile begin, EnvironmentTile destination, int character)
    {
	
        List<EnvironmentTile> result = null;

		//If both positions are viable
            ...
            if (directConnection == null)
            {
                ...
                while (mToBeTested.Count > 0 && currentNode != destination)
                {
                    ...
                    if (mToBeTested.Count > 0)
                    {
                        ...
                        // Check each neighbour, if it is accessible and the type of tile needed, and hasn't already been 
                        ...
                        for (int count = 0; count < currentNode.Connections.Count; ++count)
                        {
							...
							//Check for whether the tile is accessible and not visited
							if (!neighbour.Visited && neighbour.IsAccessible)
							{
								//Check character types for their specific tile types thats are avaliable
								if(character == 0)
								{
									mToBeTested.Add(neighbour);
								}
								//Player
								else if (character == 1 && !neighbour.isPaddock)
								{
									mToBeTested.Add(neighbour);
								}
								//Visitor
								else if (character == 2 && neighbour.isPath)
								{
									mToBeTested.Add(neighbour);
								}
								//Dog
								else if (character == 3 && neighbour.isPaddock)
								{
									mToBeTested.Add(neighbour);
								}
